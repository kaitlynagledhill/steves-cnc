...
export default function Home() {
  ...
  const fuse = useMemo(
    () => new Fuse(shuffledDesigns, { keys: ["name", "category"], threshold: 0.35 }),
    [shuffledDesigns]
  );

  const filteredDesigns = useMemo(() => {
    let results = shuffledDesigns;
    if (showFavoritesOnly) results = results.filter((d) => favorites.includes(d.id));
    ...
    return results;
  }, [shuffledDesigns, favorites, showFavoritesOnly, selectedCategory, debouncedSearch, fuse]);
...
<div className="flex flex-col items-center gap-3 w-full max-w-sm">
            ...
            {/* Saved pill button */}
  <button
    onClick={() => { setShowFavoritesOnly((v) => !v); setSelectedCategory(""); }}
  ...
  >
              <Heart
                size={12}
                className={showFavoritesOnly ? "fill-white stroke-white" : "stroke-stone-400"}
              />
...
label={t.allDesigns}
              ...
              {categories.map((c) => (
                <SidebarItem
                  ...
                  onClick={() => { setSelectedCategory(c.name); setShowFavoritesOnly(false); }}
                />
              ))}
            </nav>
          </aside>
 ...
 {showFavoritesOnly && (
  <div className="mb-5 flex items-center justify-between">
    ...
    >
      ← Back to all
            ...
            {(selectedCategory || showFavoritesOnly || debouncedSearch) && (
              <div className="mb-5 flex items-center gap-2">
                ...
                <span className="text-stone-300 text-sm">· {filteredDesigns.length}</span>
                <button
                  onClick={() => { setSelectedCategory(""); setShowFavoritesOnly(false); setSearch(""); }}
            ...
            {noResults ? (
              <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
                {showFavoritesOnly && <Heart size={32} strokeWidth={1.2} className="text-stone-200" />}
      ...
      <Link href="/admin" aria-hidden="true" tabIndex={-1} className="fixed bottom-0 right-0 w-10 h-10" />
    </>
  );
}
...
function SidebarItem({
  ...
  return (
    ...
    >
      ...
      {count !== undefined && (
        <span className={`text-xs ml-2 flex-shrink-0 tabular-nums
...
function MobilePill({ label, active, onClick }: {
